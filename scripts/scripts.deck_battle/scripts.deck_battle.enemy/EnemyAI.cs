...
namespace Enemy
{
    public class EnemyAI : MonoBehaviour
    {
        ...
        public static void PlayTopCard()
        {
            ...
            if(INSTANCE.cards.Count > 8)
            {
                if(INSTANCE.cards[0] is IAmendment)
                {
                    if (TurnManager.LastCardPlayed == null)
                    {
                        HealthManager.AttackPlayer(0);
                    }
                    else
                    {
                        HealthManager.AttackPlayer(((IAmendment)INSTANCE.cards[0]).GetPowerForCard(TurnManager.LastCardPlayed));
                    }
                ...
                {
                    HealthManager.AttackPlayer(((IAttackCard)INSTANCE.cards[0]).GetAttackingPower());
            ...
            if (TurnManager.LastCardPlayed != null && TurnManager.LastCardPlayed is AttackCard && TurnManager.moves == 0 && Random.Range(0, 100) < 85)
            {
